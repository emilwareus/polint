...
mod tests {
    ...
    fn absent_param_dead_branch_is_folded() {
        ...
        // branch, so it emits no value-flow (`PointsTo`) edge there — while
        ...
        let value_flow_resolves = |site: &str, tgt: &str| {
            db.call_targets()
                .iter()
                .any(|t| site_src(db, t).contains(site) && target_src(db, t).contains(tgt))
        };
        assert!(
    ...
    fn value_flow_array_model_pop_and_index_stay_precise() {
        ...
        // These resolve through the points-to value flow.
        let dir = tempdir().unwrap();
        let root = dir.path();
        std::fs::write(
            root.join("arr.js"),
    ...
    #[test]
    fn whole_graph_keeps_edges_from_loaded_dead_code() {
        let dir = tempdir().unwrap();
        let root = dir.path();
        // `deadFn` is loaded but never invoked. Its resolved internal call remains
        // part of polint's honest whole-repository result.
        std::fs::write(
            root.join("dead.js"),
        ...
        assert!(
            db.call_targets()
                .iter()
                .any(|target| target.caller == dead_fn.id),
            "loaded deadFn must remain visible instead of being pruned"
        );
    }
