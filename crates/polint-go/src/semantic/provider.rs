...
use crate::lifecycle::{GoAnalysisConfig, go_files};
use crate::semantic::cache_key::{
    GoSemanticCacheInputs, go_semantic_input_digest, go_semantic_provider_parameter_digest,
};
use crate::semantic::client::{GoSemanticClient, GoSemanticClientError, GoSemanticClientRun};
...
use crate::semantic::lower::lower_go_semantic;
use crate::semantic::process::GoSemanticProcessError;
...
fn replace_go_semantic_facts(
    db: &mut dyn FactDatabase,
    output: GoSemanticFactsOutput,
) -> Result<crate::semantic::store::GoSemanticStoreReport, crate::error::AnalysisError> {
    let interner = db.stable_key_interner();
