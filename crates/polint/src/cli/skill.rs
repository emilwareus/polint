...
symbol facts use typed package information when root `go.mod` setup works; setup
gaps are reported as `polint/capability` diagnostics. Symbol/reference facts are
not call graph, CFG, dataflow, coverage, or Go SSA facts.
