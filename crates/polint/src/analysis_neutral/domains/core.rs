...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum InitializednessDomain {
    Bottom,
    Initialized,
    Uninitialized,
    MaybeUninitialized,
...
impl ConstantDomain {
    pub fn from_literal(literal: ConstantLiteral) -> Self {
        Self::Values(BTreeSet::from([literal]))
    }

    pub fn from_literals(literals: impl IntoIterator<Item = ConstantLiteral>) -> Self {
...
impl StringDomain {
    ...
    pub fn from_literals<const N: usize>(literals: [&str; N]) -> Self {
        let values = literals
            .into_iter()
            .map(str::to_string)
            .collect::<BTreeSet<_>>();
...
mod tests {
    use super::*;
    use crate::analysis_neutral::domains::lattice::{
        AbstractDomain, TopReason, WidenFuel, WidenSite,
    };

    fn loop_site() -> WidenSite {
        WidenSite {
            stable_key: crate::internal_core::stable_key_for_test("body:test:block:loop"),
        }
    }
