...
use crate::analysis_neutral::reachability::store::REACHABILITY_PROVIDER_ID;
...
const UNRESOLVED_TARGET: crate::internal_core::FunctionId =
    crate::internal_core::FunctionId::from_raw(u64::MAX);
...
const UNRESOLVED_FILE: crate::internal_core::FileId =
    crate::internal_core::FileId::from_raw(u32::MAX);
...
pub fn discover_reachability_roots(
...
fn exported_root(db: &impl AnalysisHost, function: &FunctionFact) -> Option<ReachabilityRootFact> {
...
fn function_identity_for(db: &impl AnalysisHost, function: &FunctionFact) -> String {
...
fn function_identity_for_target(
    db: &impl AnalysisHost,
    target: crate::internal_core::FunctionId,
) -> Option<String> {
    db.functions()
        .iter()
...
fn go_package_name(db: &impl AnalysisHost, function: &FunctionFact) -> Option<String> {
    db.packages()
        .iter()
...
fn prefix_matches(db: &impl AnalysisHost, function: &FunctionFact, prefix: &str) -> bool {
...
fn unresolved_span() -> crate::internal_core::Span {
    crate::internal_core::Span::point(UNRESOLVED_FILE, 0, 0)
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::analysis_api::{FunctionFact, PackageFact};
    use crate::analysis_neutral::AnalysisHost;
    ...
    use crate::internal_core::{FileId, FunctionId, Language, PackageId, Span, SymbolId};
    ...
    fn entrypoint_test_kind_maps_to_test_root_with_inherited_metadata() {
        ...
        let entrypoint = EntrypointFact {
            ...
            stable_key: crate::internal_core::stable_key_for_test("entrypoint:jest:x"),
        };
    ...
    fn non_test_entrypoint_kind_maps_to_framework_entrypoint_root() {
        ...
        let entrypoint = EntrypointFact {
            ...
            provider_id: "polint.entrypoints".to_string(),
            stable_key: crate::internal_core::stable_key_for_test("entrypoint:express:get:/"),
        };
        db.replace_entrypoint_facts(EntrypointOutput {
            entrypoints: vec![entrypoint],
            trust_boundaries: Vec::new(),
