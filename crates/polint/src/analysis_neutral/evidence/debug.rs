...
use crate::analysis_neutral::evidence::facts::{EvidenceExpansion, EvidenceStatus};
    ...
    EvidenceDebugReport {
        ...
        statuses: EvidenceDebugStatuses {
            exact: store
                ...
                .filter(|edge| {
                    edge.precision
                        == crate::analysis_neutral::evidence::facts::EvidencePrecision::Exact
                })
                .count(),
            partial: store
                .paths()
            ...
            extension_backed: store
                ...
                .filter(|edge| {
                    edge.provenance
                        == crate::analysis_neutral::evidence::facts::EvidenceProvenance::Extension
                })
                .count(),
...
mod tests {
    ...
    fn debug_report_is_deterministic_compact_and_private() {
        ...
        let first =
            evidence_debug_report(&store, &crate::internal_core::test_stable_key_interner());
        let second =
            evidence_debug_report(&store, &crate::internal_core::test_stable_key_interner());
    ...
    fn debug_budget_caps_are_aggregated_across_replay_keys() {
        ...
        output.bundles.push(EvidenceBundleFact {
            ...
            diagnostic_stable_key: crate::internal_core::stable_key_for_test("diag:2"),
            ...
            stable_key: crate::internal_core::stable_key_for_test("bundle:larger"),
        });
        output.replay_keys.push(EvidenceReplayKeyFact {
            ...
            stable_key: crate::internal_core::stable_key_for_test("replay:larger"),
        });
        let store =
