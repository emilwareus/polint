...
pub fn validate_extension_output(
    ...
    let mut accepted_keys = BTreeSet::<crate::internal_core::StableKeyId>::new();
...
fn rejection_reason(
    ...
    accepted_keys: &BTreeSet<crate::internal_core::StableKeyId>,
    ...
    if let Some(id) = value.strip_prefix("call_site:") {
        return id.parse::<u64>().map_or_else(
            ...
            |id| {
                db.call_sites()
                    .iter()
                    .any(|site| site.id == crate::analysis_neutral::ids::CallSiteId(id))
            },
        );
    }
...
fn resolve_function_ref(db: &(impl AnalysisHost + ?Sized), value: &str) -> bool {
    value
        ...
        .is_some_and(|id| {
            db.functions()
                .iter()
                .any(|function| function.id == crate::internal_core::FunctionId::from_raw(id))
        })
}
...
fn resolve_symbol_ref(db: &(impl AnalysisHost + ?Sized), value: &str) -> bool {
    value
        ...
        .is_some_and(|id| {
            db.symbols()
                .iter()
                .any(|symbol| symbol.id == crate::internal_core::SymbolId::from_raw(id))
        })
}
...
mod tests {
    ...
    fn test_place(interner: &crate::internal_core::StableKeyInterner, id: u64) -> PlaceFact {
        PlaceFact {
            id: PlaceId(id),
            language: Language::TypeScript,
