...
pub fn validate_summaries(db: &impl AnalysisHost, diagnostics: &mut Vec<Diagnostic>) {
        ...
        .map(|function| {
            (
                ...
                store
                    ...
                    .filter(|target| {
                        target.status
                            == crate::analysis_neutral::calls::facts::CallTargetStatus::Resolved
                    })
                    .count(),
            )
        })
...
mod tests {
    ...
    fn diagnostic_reasons(diagnostics: &[crate::internal_core::Diagnostic]) -> Vec<&str> {
        diagnostics
            .iter()
