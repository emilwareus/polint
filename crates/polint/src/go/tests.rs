...
use crate::core::{AnalysisDb, BranchObligation, CachedFileAnalysis, Language};
...
#[test]
fn go_syntax_caches_use_only_declared_analysis_settings() {
    let cache_root = unique_cache_root();
    let cache = crate::cache::Cache::new(cache_root.join("analysis"), true);
    let plan = AnalysisPlan::empty();
    let source = "package main\nfunc main() {}\n";
    ...
    let first_key = super::adapter::go_syntax_file_cache_key(
        first.files().first().expect("Go source file"),
    ...
    for (field, mutated) in rule_only_config_mutations(&baseline) {
        let mutated_settings = analysis_settings_hash(&mutated, AnalysisSettingsScope::GoSyntax);
        ...
        let file_key = super::adapter::go_syntax_file_cache_key(
            db.files().first().expect("Go source file"),
    ...
    let unrelated_settings = analysis_settings_hash(&unrelated, AnalysisSettingsScope::GoSyntax);
    ...
    parser_changed.config.languages.go.insert(
