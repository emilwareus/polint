...
pub(crate) fn analyze_with_plan_options_and_cache_stats(
    ...
    let layer_key = ts_syntax_layer_key(&files, analysis_settings_digest);
    ...
    match read.status {
        ...
        | LayerCacheReadStatus::InvalidEvicted) => {
            ...
            let manifest = match ts_syntax_layer_manifest(layer_key, dependencies, &payload) {
                Ok(manifest) => manifest,
                Err(error) => {
                    write_diagnostics.push(cache_write_diagnostic("ts syntax layer", error));
...
fn ts_syntax_layer_key(files: &[&SourceFile], analysis_settings_digest: &Digest) -> LayerKey {
...
fn validate_syntax_layer_payload(
    ...
    let actual = payload
        ...
        .collect::<Vec<_>>();
    actual == expected
        && manifest.output_digest == ts_syntax_output_digest_for_payload(payload)
...
pub(super) fn syntax_provider_manifest_digest() -> Digest {
    let manifest = crate::analysis_kernel::AnalysisKernel::provider_manifests()
        .iter()
        .find(|manifest| manifest.id == TS_PROVIDER_ID)
        .expect("TS syntax provider manifest");
...
fn analyze_ts_source_file(
    ...
    let diagnostics = match parse_ts_file(&mut local_db, local_file) {
...
mod layer_run_metadata_path_tests {
    ...
    fn ts_layer_run_metadata_is_identical_across_hit_miss_disabled_invalid_read_and_failed_write() {
        ...
        corrupt_only_file(&cache_root.join("layers/blobs"));
    ...
    #[test]
    fn ts_per_file_cache_write_warning_is_run_local_and_excluded_from_syntax_identity() {
        let scratch = tempfile::TempDir::new().expect("scratch directory");
