...
mod tests {
    ...
    fn normalized_assigns_dense_ids_after_stable_key_sort() {
        let normalized = sample_output().normalized(&polint_core::test_stable_key_interner());
        ...
        assert_eq!(
            ...
            polint_core::stable_key_for_test("edge|copy_edge|a")
        );
        assert_eq!(normalized.derived_edges[0].id, DerivedEdgeId(0));
        assert_eq!(
            normalized.derived_edges[1].stable_key,
            polint_core::stable_key_for_test("edge|copy_edge|b")
        );
    ...
    fn normalized_is_shuffle_stable() {
        ...
        let a = base.normalized(&polint_core::test_stable_key_interner());
        let b = shuffled.normalized(&polint_core::test_stable_key_interner());
    ...
    fn from_output_builds_constraint_kind_index() {
        let store =
            SolverStore::from_output(sample_output(), &polint_core::test_stable_key_interner())
                .expect("store");
    ...
    fn from_output_preserves_run_level_budget_reasons_without_edge_rows() {
        ...
        let store = SolverStore::from_output(output, &polint_core::test_stable_key_interner())
            .expect("store");
    ...
    fn from_output_rejects_budget_reasons_without_budget_exceeded_status() {
        ...
        let error = SolverStore::from_output(output, &polint_core::test_stable_key_interner())
