...
impl_stable_keyed!(
    crate::cfg::facts::CfgFunctionFact,
    crate::cfg::facts::CfgNodeFact,
    crate::cfg::facts::BasicBlockFact,
    crate::cfg::facts::CfgEdgeFact,
    crate::cfg::facts::ReachabilityFact,
    crate::cfg::facts::DominatorFact,
    crate::cfg::facts::PostDominatorFact,
    crate::cfg::facts::ControlDependenceFact,
    crate::cfg::facts::UnsupportedControlFlowFact,
);
...
fn function_key_map(output: &CfgOutput) -> BTreeMap<CfgFunctionId, polint_core::StableKeyId> {
    output
        .functions
        .iter()
...
fn node_key_map(output: &CfgOutput) -> BTreeMap<CfgNodeId, polint_core::StableKeyId> {
    output
        .nodes
        .iter()
        .map(|row| (row.id, row.stable_key))
...
fn block_key_map(output: &CfgOutput) -> BTreeMap<BasicBlockId, polint_core::StableKeyId> {
    output
        .blocks
        .iter()
        .map(|row| (row.id, row.stable_key))
...
fn edge_key_map(output: &CfgOutput) -> BTreeMap<CfgEdgeId, polint_core::StableKeyId> {
    output
        .edges
        .iter()
        .map(|row| (row.id, row.stable_key))
...
fn stable_function_key<'a>(
    interner: &polint_core::StableKeyInterner,
    keys: &BTreeMap<CfgFunctionId, polint_core::StableKeyId>,
    id: CfgFunctionId,
) -> Cow<'a, str> {
    keys.get(&id)
...
fn stable_node_key<'a>(
    interner: &polint_core::StableKeyInterner,
    keys: &BTreeMap<CfgNodeId, polint_core::StableKeyId>,
    id: CfgNodeId,
) -> Cow<'a, str> {
    keys.get(&id)
        .map(|key| Cow::Owned(interner.resolve(*key).to_string()))
...
fn stable_block_key<'a>(
    interner: &polint_core::StableKeyInterner,
    keys: &BTreeMap<BasicBlockId, polint_core::StableKeyId>,
    id: BasicBlockId,
) -> Cow<'a, str> {
    keys.get(&id)
        .map(|key| Cow::Owned(interner.resolve(*key).to_string()))
...
fn stable_edge_key<'a>(
    interner: &polint_core::StableKeyInterner,
    keys: &BTreeMap<CfgEdgeId, polint_core::StableKeyId>,
    id: CfgEdgeId,
) -> Cow<'a, str> {
    keys.get(&id)
        .map(|key| Cow::Owned(interner.resolve(*key).to_string()))
...
fn span_part(span: &polint_core::Span) -> String {
    format!(
        "{}:{}..{}:{}@{}..{}",
        span.start_line,
        span.start_col,
...
fn optional_span_part(span: Option<&polint_core::Span>) -> Cow<'_, str> {
