...
use crate::calls::cache_key::calls_provider_parameter_digest;
use crate::calls::direct::resolve_direct_call_targets;
use crate::calls::extract::extract_call_sites;
...
pub fn derive_calls_with_cache_stats(
    ...
    let resolved_sites = targets
        .iter()
        .filter(|target| target.status == crate::calls::facts::CallTargetStatus::Resolved)
        .map(|target| target.site)
        .collect::<BTreeSet<_>>();
    for site in &mut sites {
        if resolved_sites.contains(&site.id) {
            site.status = crate::calls::facts::CallTargetStatus::Resolved;
            site.precision = crate::calls::facts::CallPrecision::SetupAware;
        }
    }
...
fn callee_part(callee: &crate::calls::facts::CallCallee) -> String {
    match callee {
        crate::calls::facts::CallCallee::Identifier { name, .. } => {
            format!("identifier:{name}")
        }
        crate::calls::facts::CallCallee::Member { property, .. } => {
            format!("member:{property}")
        }
        crate::calls::facts::CallCallee::Index { .. } => "index".to_string(),
        crate::calls::facts::CallCallee::Super => "super".to_string(),
        crate::calls::facts::CallCallee::Import => "import".to_string(),
        crate::calls::facts::CallCallee::FunctionValue { .. } => "function_value".to_string(),
        crate::calls::facts::CallCallee::Constructor { name, .. } => {
            format!("constructor:{}", name.as_deref().unwrap_or("unknown"))
        }
        crate::calls::facts::CallCallee::Unknown { reason } => {
