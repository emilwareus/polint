...
fn body_for_place<'db>(
    place: &PlaceFact,
    body_by_id: &BTreeMap<crate::ids::MirBodyId, &'db MirBody>,
...
fn collect_values_for_operation(
    ...
    body_by_id: &BTreeMap<crate::ids::MirBodyId, &MirBody>,
...
"#;
        ...
        for needle in [
            ...
            operations.push(operation(
                ...
                MirOperationKind::Branch {
                    predicate: crate::ids::MirPredicateId(operations.len() as u64),
    ...
    fn span_for(source: &str, file: FileId, start: &str, end: &str) -> Span {
        ...
        let end_byte = source[start_byte..]
            ...
            .expect(end);
        polint_core::span_from_byte_range(file, source, start_byte, end_byte)
    }
}
