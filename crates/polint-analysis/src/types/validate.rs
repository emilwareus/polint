...
fn pt_var_id(
    ...
    if let Some(place) = value.strip_prefix("place:") {
        place
            ...
            .ok()
            .map(PlaceId)
            .map(crate::points_to::vars::place_var)
    } else if let Some(path) = value.strip_prefix("access_path:") {
        path.parse()
            .ok()
            .and_then(|id| extension_refs.access_path(fact, id))
            .map(crate::points_to::vars::access_path_var)
    } else if let Some(allocation) = value.strip_prefix("allocation:") {
        allocation
            .parse()
            .ok()
            .and_then(|id| extension_refs.allocation(fact, id))
            .map(crate::points_to::vars::allocation_var)
    } else {
        None
    }
}
...
fn object_token_id(
    ...
    if let Some(allocation) = value.strip_prefix("allocation:") {
        ...
        abstract_value
            ...
            .map(crate::points_to::vars::abstract_value_object)
    } else {
        None
    }
}
...
mod tests {
    use super::*;
    use crate::aliases::facts::{AliasAnswerFact, AliasReason};
    ...
    fn test_interner() -> polint_core::StableKeyInterner {
        ...
        let output = TypeValueAliasOutput {
            aliases: AliasOutput {
                answers: vec![AliasAnswerFact {
                    ...
                    stable_key: polint_core::stable_key_for_test("alias:native:unknown"),
                }],
            },
            ..TypeValueAliasOutput::default()
        };
        ...
        let output = TypeValueAliasOutput {
            aliases: AliasOutput {
                answers: vec![AliasAnswerFact {
                    ...
                    evidence: vec!["native-missing".to_string()],
                    precision: AliasPrecision::Unknown,
                    stable_key: polint_core::stable_key_for_test("alias:shared"),
                }],
            },
            ..TypeValueAliasOutput::default()
        };
    ...
    fn extension_relation_refs_resolve_to_extension_local_base_facts() {
        let output = TypeValueAliasOutput {
            values: crate::values::store::ValueOutput {
                allocations: vec![AllocationTokenFact {
                    ...
                    stable_key: polint_core::stable_key_for_test("allocation:a-native"),
                }],
                ..Default::default()
            },
            ..TypeValueAliasOutput::default()
        ...
        assert!(merged.points_to.constraints.iter().any(|constraint| {
            matches!(
                ...
                PointsToConstraintKind::AddressOf { dst, object }
                    if dst == crate::points_to::vars::allocation_var(extension_allocation)
                        && object
                            == crate::points_to::vars::allocation_object(
        ...
        assert!(
            merged.points_to.constraints.iter().any(|constraint| {
                matches!(
                    constraint.kind,
                    PointsToConstraintKind::AddressOf { dst, object }
                        if dst == crate::points_to::vars::allocation_var(lexical_first)
                            && object
                                == crate::points_to::vars::allocation_object(
                                    lexical_first
                                )
