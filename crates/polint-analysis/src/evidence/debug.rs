...
use crate::evidence::facts::{EvidenceExpansion, EvidenceStatus};
use crate::evidence::store::EvidenceStore;

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct EvidenceDebugReport {
    pub counts: EvidenceDebugCounts,
    pub statuses: EvidenceDebugStatuses,
    pub summary_expansion_keys: Vec<String>,
    pub summary_opaque_reasons: Vec<String>,
    pub replay_keys: Vec<String>,
    pub unknown_reasons: Vec<String>,
    pub omitted_regions: Vec<EvidenceDebugOmittedRegion>,
    pub hidden_node_count: u32,
    pub budget_caps: EvidenceDebugBudgetCaps,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct EvidenceDebugCounts {
    pub nodes: usize,
    pub edges: usize,
    pub bundles: usize,
    pub paths: usize,
...
pub struct EvidenceDebugStatuses {
    pub exact: usize,
    pub partial: usize,
    pub unknown: usize,
    pub summary_backed: usize,
    pub extension_backed: usize,
    pub budget_limited: usize,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct EvidenceDebugOmittedRegion {
...
pub struct EvidenceDebugBudgetCaps {
    pub max_paths: u32,
    pub max_nodes: u32,
    pub max_edges: u32,
    pub max_depth: u32,
...
mod tests {
    ...
    fn debug_report_is_deterministic_compact_and_private() {
        ...
        let first = evidence_debug_report(&store, &polint_core::test_stable_key_interner());
        let second = evidence_debug_report(&store, &polint_core::test_stable_key_interner());
    ...
    fn debug_budget_caps_are_aggregated_across_replay_keys() {
        ...
        output.bundles.push(EvidenceBundleFact {
            ...
            diagnostic_stable_key: polint_core::stable_key_for_test("diag:2"),
            ...
            stable_key: polint_core::stable_key_for_test("bundle:larger"),
        });
        output.replay_keys.push(EvidenceReplayKeyFact {
            ...
            stable_key: polint_core::stable_key_for_test("replay:larger"),
        });
        let store = EvidenceStore::from_output(output, &polint_core::test_stable_key_interner())
            .expect("valid evidence");

        let report = evidence_debug_report(&store, &polint_core::test_stable_key_interner());
    ...
    fn store() -> EvidenceStore {
        EvidenceStore::from_output(output(), &polint_core::test_stable_key_interner())
            .expect("valid evidence")
    }
