...
fn entrypoint_counts(db: &impl AnalysisHost) -> EntrypointDebugCounts {
...
fn entrypoint_detail_rows(db: &impl AnalysisHost) -> Vec<EntrypointDetailRow> {
...
fn trust_boundary_detail_rows(db: &impl AnalysisHost) -> Vec<TrustBoundaryDetailRow> {
...
fn dispatch_edge_detail_rows(db: &impl AnalysisHost) -> Vec<DispatchEdgeDetailRow> {
...
fn unresolved_detail_rows(db: &impl AnalysisHost) -> Vec<UnresolvedDetailRow> {
...
fn relative_path_for(db: &impl AnalysisHost, file: FileId) -> String {
    db.file(file)
        .map(|f| f.relative_path.clone())
        .unwrap_or_else(|| "<unknown-file>".to_string())
}

fn trigger_summary(metadata: &crate::entrypoints::facts::TriggerMetadata) -> String {
...
mod tests {
    ...
    fn test_db_populated() -> LocalAnalysisDb {
        ...
        let entrypoint = EntrypointFact {
            ...
            stable_key: polint_core::stable_key_for_test("entrypoint:express:get:/api/users"),
        };
        ...
        let dispatch_edge = FrameworkDispatchEdgeFact {
            ...
            precision: EntrypointPrecision::ResolvedStatic,
            provider_id: "polint.entrypoints".to_string(),
            stable_key: polint_core::stable_key_for_test("dispatch:express:get:/api/users"),
        };
        ...
        let unresolved = UnresolvedFrameworkFact {
            ...
            provider_id: "polint.entrypoints".to_string(),
            stable_key: polint_core::stable_key_for_test("unresolved:fastify:file"),
        };
