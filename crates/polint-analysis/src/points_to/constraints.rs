...
use crate::access_paths::facts::{AccessPathFact, AccessPathProjection};
use crate::ids::{PointsToConstraintId, PtVarId};
use crate::points_to::vars;
use crate::types::store::TypeValueAliasOutput;
use crate::values::facts::{AllocationTokenFact, ValueFact, ValueKind, ValueStatus, ValueSubject};
...
impl ConstraintBuilder {
    fn collect_values(&mut self, interner: &polint_core::StableKeyInterner, values: &[ValueFact]) {
        for value in values {
    ...
    fn push(&mut self, interner: &polint_core::StableKeyInterner, kind: PointsToConstraintKind) {
    ...
    fn finish(mut self, interner: &polint_core::StableKeyInterner) -> Vec<PointsToConstraintFact> {
...
mod tests {
    use super::*;
    use crate::access_paths::facts::AccessPathStatus;
    use crate::access_paths::store::AccessPathOutput;
    use crate::ids::{AccessPathId, AllocationTokenId, MirBodyId, ValueFactId};
    use crate::types::store::TypeValueAliasOutput;
    use crate::values::facts::{ValuePrecision, ValueProvenance, ValueStatus};
    ...
    fn derives_core_constraints_from_values_and_access_paths() {
        let output = TypeValueAliasOutput {
            values: ValueOutput {
                values: vec![
                    value(
                        ValueFactId(0),
                        ValueSubject::Place(crate::ids::PlaceId(1)),
                        ValueKind::Object(AllocationTokenId(7)),
                    ),
                    value(
                        ValueFactId(1),
                        ValueSubject::Place(crate::ids::PlaceId(2)),
                        ValueKind::PlaceRef(crate::ids::PlaceId(1)),
                    ),
                    value(
                        ValueFactId(2),
                        ValueSubject::Place(crate::ids::PlaceId(3)),
                        ValueKind::CallReturn(crate::ids::PlaceId(2)),
                    ),
                ],
                allocations: Vec::new(),
            },
            access_paths: AccessPathOutput {
                access_paths: vec![
                    ...
                    path(
                        AccessPathId(3),
                        AccessPathProjection::CallReturn(crate::ids::CallSiteId(9)),
                    ),
                ],
            },
        ...
        let constraints =
            derive_points_to_constraints(&polint_core::test_stable_key_interner(), &output);
        assert!(
            constraints
                .iter()
        ...
        let output = TypeValueAliasOutput {
            values: ValueOutput {
                values: vec![value_with_status(
                    ValueFactId(0),
                    ValueSubject::Place(crate::ids::PlaceId(3)),
                    ValueKind::CallReturn(crate::ids::PlaceId(3)),
                    ValueStatus::Unknown,
                )],
                allocations: Vec::new(),
            },
    ...
    fn value_with_status(
        ...
        ValueFact {
            id,
            subject,
            value: crate::ids::AbstractValueId(id.0),
            kind,
            ...
            stable_key: polint_core::stable_key_for_test(&format!("value:{}", id.0)),
        }
    }
