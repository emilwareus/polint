mod formats;
use crate::DYNAMIC_IMPORT_SPECIFIER;
use crate::repo_fs::{
...
use polint_analysis::module_graph::model::{
...
use polint_core::{FileId, Language, ModuleNodeId, StableKeyInterner};
...
fn intern_module_graph_stable_key(key: impl Into<String>) -> polint_core::StableKeyId {
    MODULE_GRAPH_STABLE_KEYS.with(|slot| {
        slot.borrow()
            .as_ref()
...
fn emit_tsconfig_overlays(output: &mut TopologyOutput, root: &Path, ts_files: &[&SourceFile]) {
    ...
    for file in ts_files {
        ...
        if let Some(config) = nearest_tsconfig_path(root, &absolute)
            && let Some(relative) = crate::repo_fs::normalize_repo_relative_path(root, &config)
        {
