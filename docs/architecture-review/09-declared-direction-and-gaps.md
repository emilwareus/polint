...
| 12 | **incremental-query-engine** | Native layered incrementality: `InputSnapshot` → layer cache → `DependencyIndex` → `InvalidationPlan` → demand queries. Explicitly *not* Salsa-first | **Partial — top half only** | Vocabulary built at `analysis_kernel/incremental/{input_snapshot,keys,change_set,dependency_index,invalidation,layer_cache,quarantine,demand}.rs` (**path diverges** from the specified `analysis/incremental/`). The demand-query layer — `analysis/demand/` (2,188 LOC: `query.rs`, `context.rs`, `scc.rs`, `quarantine.rs`, `trace.rs`) — is **referenced nowhere**; its only mentions are five string literals in `analysis_kernel/provider.rs:1816-1821`. |
