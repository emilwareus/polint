...
Some capabilities already have public `RuleCtx` accessors and adapter-produced
facts. Others are still mostly aspirational.
...
2. Add `RuleCtx::cfg(function_id) -> Option<&ControlFlowGraph>`.
...
3. Add `RuleCtx::coverage_for_file(file_id)`,
   `RuleCtx::coverage_for_function(function_id)`, and
   `RuleCtx::coverage_for_branch(branch_id)`.
...
3. Add `RuleCtx::resolved_imports()` and `RuleCtx::module_graph()`.
...
6. For Python later, start with lexical/direct call names and import-resolved
   ...
   confidence.
7. Add `RuleCtx::call_edges()` and `RuleCtx::calls_from(function_id)`.
...
2. Define stable symbol keys from language, package/module path, file, lexical
   owner, name, and span.
3. Add `RuleCtx::symbols()`, `RuleCtx::references()`,
   `RuleCtx::references_to(symbol_id)`, and `RuleCtx::definition(symbol_id)`.
...
2. Add `RuleCtx::test_metrics_for_file(file_id)` and
   `RuleCtx::test_metrics_for_function(function_id)`.
...
Fulfill `.cfg()` with an honest intra-procedural control-flow model.
...
Fulfill `.coverage_facts()` by ingesting external coverage reports.
...
Fulfill `.test_suite_metrics()` with first-class metrics beyond raw `TestFact`.
...
8. Python adapter with a declared subset, then expand toward parity.
9. Java adapter with a declared subset, then expand toward parity.
