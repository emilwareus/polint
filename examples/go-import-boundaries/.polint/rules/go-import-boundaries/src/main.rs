                    ...
                    })
                {
                    diagnostics.push(
                        Diagnostic::error(
                            ...
                            format!("Go import `{}` violates the local import boundary.", import.path),
                        )
                        .with_evidence("import", import.path.clone())
                        .with_help("Move this dependency behind an allowed interface or update the local boundary config."),
                    );
                }
            }
        }
