...
Next milestone requirements should start from a fresh `.planning/REQUIREMENTS.md` via `/gsd-new-milestone`.
...
- [x] **FND-01**: The repository contains a Rust 2024 workspace with crates for CLI, config, diagnostics, filesystem, cache, core, SDK, Go adapter, TS adapter, graph helpers, and rules.
...
- **SEM-03**: Add more language adapters after Go and TS/JS stabilize.

## Out of Scope

| Feature | Reason |
|---------|--------|
| Comprehensive bundled rule pack | The framework exists to make custom repo-local policy easy. |
...
| Full automatic compilation of repo-local Rust rules in v1 | Scaffolding and explicit Cargo integration are the first deliverable. |
